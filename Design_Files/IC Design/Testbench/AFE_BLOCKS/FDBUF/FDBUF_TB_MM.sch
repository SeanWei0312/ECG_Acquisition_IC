...
N 560 -1220 560 -1200 {lab=FDBUF_SEL}
...
shell mkdir -p /foss/designs/ECG_Acquisition_IC/Measurement_Results/IC_Simulation/AFE_BLOCKS/FDBUF/\{$mode\}.Result_txt
shell rm -f /foss/designs/ECG_Acquisition_IC/Measurement_Results/IC_Simulation/AFE_BLOCKS/FDBUF/\{$mode\}.Result_txt/\{$tag\}.*_mc_summary.txt
...
let out_cm_error=fdbuf_out_cm-v(REF)
...
let fdbuf_gain=mag(fdbuf_diff/vin_diff)

meas ac gain10 find fdbuf_gain at=10
meas ac gain150 find fdbuf_gain at=150

let norm_db=20*log10(fdbuf_gain/$&gain10)

meas ac bw3db when norm_db=-3 fall=1

meas ac ad60 find fdbuf_gain at=60
meas ac ad150 find fdbuf_gain at=150

let gain_error_pct=100*($&gain10/$&FDBUF_GAIN_TARGET_SET-1)
...
echo $&run $ibfdc_val $ibcmfb_val $idd_val $power_val $outcmerr_val $vos_val $gain10_val $gainerr_val $gain150_val $bw3db_val $cmrr60_val $cmrr150_val >> /foss/designs/ECG_Acquisition_IC/Measurement_Results/IC_Simulation/AFE_BLOCKS/FDBUF/\{$mode\}.Result_txt/\{$tag\}.fdbuf_mc_summary.txt

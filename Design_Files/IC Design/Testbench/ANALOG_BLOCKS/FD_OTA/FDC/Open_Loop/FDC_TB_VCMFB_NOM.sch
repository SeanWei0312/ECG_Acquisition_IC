...
C {vsource.sym} 200 -630 0 0 {name=VDIFF       value="dc \{VOSDC\} ac 1"          savecurrent=false}
...
C {devices/code_shown.sym} 80 -520 0 0 {name=MODELS
only_toplevel=true
...
.csparam PROC_ID=0
"}
C {devices/code_shown.sym} 880 -970 0 0 {name=NGSPICE only_toplevel=true
value="
.control
destroy all
save all
...
shell rm -f /foss/designs/ECG_Acquisition_IC/Measurement_Results/IC_Simulation/ANALOG_BLOCKS/FD_OTA/FDC/NOM.Result_txt/NOM.vcmfb_sweep.txt
...
* VCMFB sweep
dc VCMFBBIAS 0 $&avdd_run 1e-4

let sweep_vcmfb  = v(cmfb)-v(agnd)
let sweep_voutp  = v(outp)-v(agnd)
let sweep_voutn  = v(outn)-v(agnd)
let sweep_voutcm = (sweep_voutp+sweep_voutn)/2
let sweep_vod    = sweep_voutp-sweep_voutn

setscale sweep_vcmfb

wrdata /foss/designs/ECG_Acquisition_IC/Measurement_Results/IC_Simulation/ANALOG_BLOCKS/FD_OTA/FDC/NOM.Result_txt/NOM.vcmfb_sweep.txt sweep_voutp sweep_voutn sweep_voutcm sweep_vod
...
C {vsource.sym} 360 -630 0 0 {name=VCMFBBIAS   value="dc \{CMFB_BIAS\} ac 0"      savecurrent=false}
...
C {ECG_Acquisition_IC/Design_Files/IC Design/Schematic/ANALOG_BLOCKS/FD_OTA/FDC/FDC.sym} 220 -880 0 0 {name=xFDC1}

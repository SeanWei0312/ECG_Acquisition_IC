...
let buffer_out_diff=v(BUFFER_OUTP)-v(BUFFER_OUTN)
let buffer_cm_error=buffer_out_cm-vref
...
let buffer_r=real(buffer_diff)
let buffer_i=imag(buffer_diff)
...
let out_r=real(buffer_diff)
let out_i=imag(buffer_diff)
...
noise v(BUFFER_OUTP,BUFFER_OUTN) VDIFF dec 100 0.01 10k
...
let int_diff=v(BUFFER_OUTP)-v(BUFFER_OUTN)

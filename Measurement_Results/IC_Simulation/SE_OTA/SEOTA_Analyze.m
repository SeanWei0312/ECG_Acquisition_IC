...
MARK_FREQ_HZ = [1 60 150 1e3];
NOISE_MARK_HZ = [NOISE_BAND_HZ(1) 60 NOISE_BAND_HZ(2)];

%% Operating point
op = cols(numdata(fullfile(baseDir,TAG + ".ol_op.txt")),8);
op = op(end,:);
vdd_V = op(2);
vinCm_V = 0.5*(op(3) + op(4));
idd_A = abs(op(7));
ibias_A = abs(op(8));
totalCurrent_A = idd_A + ibias_A;
...
noise = cols(numdata(fullfile(baseDir,TAG + ".ol_noise.txt")),2);
fNoise_Hz = noise(:,1);
outNoise_VrtHz = abs(noise(:,2));
...
%% Open-loop: VTC
vtc = cols(numdata(fullfile(baseDir,TAG + ".ol_vtc.txt")),6);
vtcVinDiff_V = vtc(:,1);
vtcVout_V = vtc(:,5);
offset_V = zeroNoJump(vtcVinDiff_V,vtcVout_V - VOUT_TARGET,0.25*vdd_V);

figure;
plot(vtcVinDiff_V*1e3,vtcVout_V,'LineWidth',1.5); hold on;
yline(VOUT_TARGET,'--','VDD/2','HandleVisibility','off');
addCursor(offset_V*1e3,VOUT_TARGET,sprintf('Vos: %.4g mV',offset_V*1e3));
ylabel('Vout (V)');
stylePlot('Vin,diff (mV)','SEOTA Open-Loop VTC');
saveFig(plotDir,'NOM.open_loop_vtc.png');

%% Closed-loop: VTC and output swing
cl = cols(numdata(fullfile(baseDir,TAG + ".cl_dc.txt")),7);
clVin_V = cl(:,1);
clVout_V = cl(:,2);
[clVin_V,idx] = sort(clVin_V);
clVout_V = clVout_V(idx);

[~,i0] = min(abs(clVin_V - VOUT_TARGET));
clGain = localSlope(clVin_V,clVout_V,VOUT_TARGET);
gainError_pct = 100*(clGain - 1);
voutDC_V = clVout_V(i0);

trackError_V = clVout_V - clVin_V;
...
if isfinite(iLow)
    ...
    inputLow_V = NaN; inputHigh_V = NaN;
    swingLow_V = NaN; swingHigh_V = NaN;
end

figure;
tiledlayout(2,1);
sgtitle('SEOTA Output Swing and Closed-Loop VTC');

nexttile;
plot(clVin_V,trackError_V*1e3,'LineWidth',1.5); hold on;
yline(TRACK_LIMIT*1e3,'--','+2mV','HandleVisibility','off');
yline(-TRACK_LIMIT*1e3,'--','-2mV','HandleVisibility','off');
if isfinite(iLow)
    xline(inputLow_V,'--',sprintf('Low: %.4g V',inputLow_V),'HandleVisibility','off');
    xline(inputHigh_V,'--',sprintf('High: %.4g V',inputHigh_V),'HandleVisibility','off');
end
...
if isfinite(iLow)
    addCursorLine(inputLow_V,clVout_V(iLow),sprintf('In low: %.4g V',inputLow_V));
    addCursorLine(inputHigh_V,clVout_V(iHigh),sprintf('In high: %.4g V',inputHigh_V));
...
tr = cols(numdata(fullfile(baseDir,TAG + ".cl_tran.txt")),7);
t_s = tr(:,1);
trVin_V = tr(:,2);
trVout_V = tr(:,3);
[srRise_Vus,srFall_Vus,tRise_s,tFall_s,risePt,fallPt] = stepMetrics(t_s,trVin_V,trVout_V,SETTLE_LIMIT);
settle_s = maxFinite([tRise_s tFall_s]);

figure;
plot(t_s*1e6,trVin_V,'--','LineWidth',1.2); hold on;
plot(t_s*1e6,trVin_V+SETTLE_LIMIT,':','LineWidth',1.0,'HandleVisibility','off');
plot(t_s*1e6,trVin_V-SETTLE_LIMIT,':','LineWidth',1.0,'HandleVisibility','off');
...
addMetricBox({sprintf('Settle: %s ns',fmt(settle_s*1e9))});
ylabel('Voltage (V)');
...
rows = [
    ...
    "Vout,DC",               "V",     fmt(voutDC_V)
    "Output swing low",      "V",     fmt(swingLow_V)
    "Output swing high",     "V",     fmt(swingHigh_V)
